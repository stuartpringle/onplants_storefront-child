...
if(sizeof($items) > 0) {
	foreach($items as $item_id => $item) {
		...
		$primary_term_product_id = $product_category[0];
...
<?php
	foreach($temp_item as $item_id => $item) {
		...
		if(in_array(125, $product_category)) {
			...
			$number_in_pack = (isset($cur_attributes['number-of-plants-in-pack']->get_options()[0]) ? $cur_attributes['number-of-plants-in-pack']->get_options()[0] : 1);
			$display_latin_name = 0;
		}

		if($product->get_type() != 'pw-gift-card' && strtolower($item['name']) != 'admin fee') {
